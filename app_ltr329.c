...
uint32_t ltr329_config(void)
{
    ...
    NRF_LOG_DEBUG("LTR-329ALS Control done"); NRF_LOG_FLUSH();
    ...
    if(err_code != NRF_SUCCESS) return err_code;
    NRF_LOG_DEBUG("LTR-329ALS Measurement Done"); NRF_LOG_FLUSH();
...
uint32_t ltr329_init(void)
{
    ...
    NRF_LOG_DEBUG("Resetting LTR-329ALS"); NRF_LOG_FLUSH();
...
bool ltr329_has_new_data(void)
{
    ...
    NRF_LOG_DEBUG("Status ... %d", raw_value); NRF_LOG_FLUSH();
